...
import { runWorkflowScript, type JSONValue, type WorkflowExecutionOptions } from "../src/runtime.ts"
import type { AppServerAgentHandle, AppServerClient } from "../src/app-server.ts"
...
function fakeHandle(result: JSONValue): AppServerAgentHandle {
  return {
...
test("composition resolves script paths and names, preserves args/results, and rejects grandchild calls", async () => {
  ...
  try {
    ...
    await writeFile(childPath, script("return { child: true, args }", "child-workflow"))
    await writeFile(nestedPath, script(`
      try {
    ...
    `)
    const execution = await runWorkflowScript(parent, { workflowDirectory: directory, fileName: join(directory, "parent.js") })
    expect(execution.result).toEqual({
      byPath: { child: true, args: { topic: "tea", list: ["a", "b"] } },
      byName: { child: true, args: { topic: "tea", list: ["a", "b"] } },
      nested: { nestedThrew: true, message: "workflow() cannot be called from within a child workflow — nesting is limited to one level. Inline the inner script or call its agents directly." },
      unknown: true,
    })
  } finally {
...
test("parent and child share agent IDs and the scheduler state", async () => {
  ...
  try {
    const childPath = join(directory, "child.js")
    await writeFile(childPath, script("return await agent('child', { model: 'gpt-5.6-luna' })", "shared-child"))
    const agentIds: string[] = []
    const appServer = {
      startAgent: async (_prompt: string, options: Record<string, JSONValue>) => {
    ...
    expect(execution.usage).toMatchObject({ agentCount: 2, liveAgentCount: 2, peakConcurrentAgents: 1 })
  } finally {
    await rm(directory, { recursive: true, force: true })
  }
})
...
test("shared queue bounds concurrency while all queued agents complete", async () => {
  ...
  expect(execution.result).toEqual(["queued-0", "queued-1", "queued-2", "queued-3", "queued-4", "queued-5", "queued-6"])
  expect(peak).toBe(2)
  expect(execution.usage).toMatchObject({ agentCount: 7, liveAgentCount: 7, replayedAgentCount: 0, peakConcurrentAgents: 2 })
})

test("lifetime, boundary, and budget caps are explicit and catchable", async () => {
  const lifetime = await runWorkflowScript(script(`
  ...
  const boundary = await runWorkflowScript(script(`
    try { await parallel([() => null, () => null, () => null]); return 'missed' }
    catch (error) { return error.message }
  `), { caps: { maxBoundaryItems: 2 } })
  expect(boundary.result).toBe("array length 3 exceeds the maximum of 2 supported across the workflow VM boundary")

  const budget = await runWorkflowScript(script(`
    try { await agent('blocked'); return 'missed' }
    catch (error) { return error.message }
  `), { budget: { total: 0 }, agent: async () => "never" })
...
test("cancellation is shared with queued child work", async () => {
  ...
  let started = false
  const promise = runWorkflowScript(script(`
  ...
  expect(execution.failures.every((failure) => failure.message.includes("cancelled"))).toBe(true)
})

test("worktree isolation propagates exact cwd and removes only clean worktrees", async () => {
  const repository = await mkdtemp(join(tmpdir(), "gpt-workflow-worktree-repo-"))
  ...
  await execFileAsync("git", ["config", "user.email", "phase5@example.test"], { cwd: repository })
  await execFileAsync("git", ["config", "user.name", "Phase 5 Test"], { cwd: repository })
  await execFileAsync("git", ["add", "."], { cwd: repository })
  await execFileAsync("git", ["commit", "-qm", "seed"], { cwd: repository })
  const received: Array<Record<string, JSONValue>> = []
  const appServer = {
    startAgent: async (_prompt: string, options: Record<string, JSONValue>) => {
  ...
  try {
    ...
    `), { appServer, workflowRunId: runId, cwd: repository })
    expect(execution.result).toBe("worktree-ok")
    expect(received[0]?.cwd).toContain(`${repository}/.verification-artifacts/worktrees/${runId}-1`)
    expect(received[0]?.sandbox).toBe("workspace-write")
    expect(received[0]?.cwd).not.toBe(repository)
    const worktrees = await execFileAsync("git", ["-C", repository, "worktree", "list", "--porcelain"], { encoding: "utf8" })
    ...
    await runWorkflowScript(script(`return await agent('dirty', { model: 'gpt-5.6-luna', isolation: 'worktree' })`), {
      appServer: dirtyAppServer,
      workflowRunId: dirtyRunId,
      cwd: repository,
    })
    const dirtyPath = `${repository}/.verification-artifacts/worktrees/${dirtyRunId}-1`
    expect(await readFile(`${dirtyPath}/dirty-marker.txt`, "utf8")).toBe("leave me")
    await unlink(`${dirtyPath}/dirty-marker.txt`)
    await execFileAsync("git", ["-C", repository, "worktree", "remove", dirtyPath], { encoding: "utf8" })
  } finally {
...
test("journal replay is byte-identical, repeatable, and invalidates the later prefix", async () => {
  ...
  try {
    ...
    expect(replay.usage).toMatchObject({ liveAgentCount: 0, replayedAgentCount: 3, subagentTokens: 0 })
    expect(changed.result).toEqual({ a: "live-1", b: "live-4", c: "live-5" })
    expect(changed.result).not.toEqual(first.result)
    expect(changed.usage).toMatchObject({ liveAgentCount: 2, replayedAgentCount: 1 })

    const lines = (await readFile(join(directory, "journal.jsonl"), "utf8"))
      .trim().split("\n").map((line) => JSON.parse(line) as { type: string; key: string })
    expect(lines).toHaveLength(10)
    const started = lines.filter((line) => line.type === "started").map((line) => line.key)
