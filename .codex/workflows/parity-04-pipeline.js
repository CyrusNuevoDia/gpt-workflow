...
const results = await pipeline(
  ...
  (prev, item, index) => {
    ...
    return agent('Reply with exactly this single lowercase word and nothing else: ' + item.key, { model: 'gpt-5.6-luna', label: 's1:' + item.key, phase: 'Stage1' })
  },
  (prev, item, index) => {
    calls.push({ stage: 2, key: item.key, index })
    return agent('Convert this word to UPPERCASE and reply with only the uppercase word, nothing else: ' + String(prev).trim(), { model: 'gpt-5.6-luna', label: 's2:' + item.key, phase: 'Stage2' })
  },
)
