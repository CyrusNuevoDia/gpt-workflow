...
const r = await parallel([
  () =>
    agent("Reply with exactly this and nothing else: gpt-5.6-luna-low-ok", {
      effort: "low",
      label: "override:gpt-5.6-luna-low",
      model: "gpt-5.6-luna",
      phase: "Overrides"
    }),
  () =>
    agent("Reply with exactly this and nothing else: gpt-5.6-terra-ok", {
      effort: "low",
      label: "override:gpt-5.6-terra",
      model: "gpt-5.6-terra",
      phase: "Overrides"
    }),
  () =>
...
check(
  "model+effort override (gpt-5.6-luna, low) completes",
  typeof r[0] === "string" && r[0].toLowerCase().indexOf("gpt-5.6-luna-low-ok") !== -1,
  JSON.stringify(r[0])
);
check(
  "model override to a different tier (gpt-5.6-terra) completes",
  typeof r[1] === "string" && r[1].toLowerCase().indexOf("gpt-5.6-terra-ok") !== -1,
  JSON.stringify(r[1])
);
check(
