...
await agent('Reply with exactly: budget-probe-ok', { model: 'gpt-5.6-luna', label: 'budget:probe', phase: 'Budget' })
...
check('INFO spent() delta across one gpt-5.6-luna agent call (recorded)', true, 'delta=' + (spentAfter - spentBefore))
...
while (budget.total && budget.remaining() > 50000 && iterations < 2) {
  iterations++
  await agent('Reply with exactly: budget-loop-ok', { model: 'gpt-5.6-luna', label: 'budget:loop-' + iterations, phase: 'Budget' })
