...
export const meta = {
  ...
  description: 'resumeFromRunId: entropy nonces expose which agents ran live vs replayed from the journal; run 3x per the PARITY.md resume protocol',
  phases: [
    { title: 'Nonces', detail: 'A fixed -> B salted -> C fixed, sequential' },
  ],
}
...
check('runner passed args.salt (this suite is one leg of a 3-run protocol)', salt !== null, 'args=' + JSON.stringify(args === undefined ? null : args))
check('INFO args arrival mode this leg', true, args === undefined ? 'undefined' : typeof args === 'string' ? 'JSON-encoded string' : typeof args)
if (salt === null) {
  return { suite: 'parity-12-resume', passed: false, checks, salt: null, nonces: null }
}
log('leg: salt=' + JSON.stringify(salt))

const rawA = await agent(
  noncePrompt('alpha — fixed prompt, before the salted call'),
  { model: 'gpt-5.6-luna', label: 'resume:A-fixed', phase: 'Nonces' },
)
const rawB = await agent(
  noncePrompt('salted ' + JSON.stringify(salt) + ' — cache-busted by changing args.salt'),
  { model: 'gpt-5.6-luna', label: 'resume:B-salted', phase: 'Nonces' },
)
const rawC = await agent(
  noncePrompt('charlie — fixed prompt, after the salted call'),
  { model: 'gpt-5.6-luna', label: 'resume:C-fixed', phase: 'Nonces' },
)
...
check('nonce A is 16-char hex entropy from Bash', typeof a === 'string' && HEX16.test(a), JSON.stringify(a))
check('nonce B is 16-char hex entropy from Bash', typeof b === 'string' && HEX16.test(b), JSON.stringify(b))
check('nonce C is 16-char hex entropy from Bash', typeof c === 'string' && HEX16.test(c), JSON.stringify(c))
check('INFO nonces echoed for the runner to diff across legs', true, JSON.stringify({ salt, a, b, c }))

const passed = checks.every(x => x.pass)
log('parity-12-resume: ' + checks.filter(x => x.pass).length + '/' + checks.length + ' in-script checks passed (cross-run checks are the runner\'s)')
return { suite: 'parity-12-resume', passed, checks, salt, nonces: { a, b, c } }
