...
export const meta = {
  description:
    "args global: verbatim passthrough (objects intact, strings stay strings), undefined when omitted, parameterizes prompts",
  name: "parity-05-args",
  phases: [{ title: "Args" }]
};
...
if (args === undefined) {
  check(
    "args is undefined when the caller omits it",
    true,
    "typeof args=" + typeof args
  );
  log("parity-05-args: no-args mode, 1/1 checks passed");
  return { checks, mode: 'no-args', passed: true, suite: 'parity-05-args' }
}
...
if (typeof args === "string") {
  ...
  check(
    "string args arrive verbatim as one string (runtime does not parse)",
    true,
    JSON.stringify(args)
  );
  let parsed = null;
  ...
  check(
    "INFO string content survives unmangled (parsed keys recorded)",
    true,
  ...
  return { checks, echoed: args, mode: 'with-string-args', passed: checks.every(c => c.pass), suite: 'parity-05-args' }
}

check(
  "args arrives as a real value, not a JSON string",
  typeof args === "object" && args !== null,
  "typeof=" + typeof args
);
check(
  "nested array survives verbatim (.map works)",
  Array.isArray(args.list) &&
    args.list.map((x) => String(x)).length === args.list.length,
  JSON.stringify(args.list)
);
check(
  "string field arrives intact",
  typeof args.topic === "string",
  JSON.stringify(args.topic)
);
check(
  "number field arrives intact",
...
const sentence = await agent(
  ...
  { label: "args:consumer", model: "gpt-5.6-luna", phase: "Args" }
);
check(
  "args can parameterize agent prompts",
...
return { checks, echoed: args, mode: 'with-args', passed, suite: 'parity-05-args' }
