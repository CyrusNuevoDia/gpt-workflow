...
import type { JSONValue, JSONObject } from "./runtime.ts"
...
export class WorkflowJournal {
  ...
  keyFor(previousKey: string, prompt: string, options: JSONObject | undefined): string {
    const input = stableStringify({ previousKey, prompt, options: options ?? null })
  ...
  private append(line: JournalLine): Promise<void> {
    this.writeTail = this.writeTail.then(() => appendFile(this.path, `${JSON.stringify(line)}\n`))
...
function parseReplayEntries(source: string): WorkflowJournalEntry[] {
  ...
  for (const entries of starts.values()) {
    for (const started of entries) {
      const matching = results.get(started.key)?.findIndex((result) => result.agentId === started.agentId) ?? -1
      if (matching < 0) continue
      const result = results.get(started.key)!.splice(matching, 1)[0]!
      replay.push({ key: started.key, agentId: started.agentId, result: result.result })
    }
  }
  return replay
}
...
function isJournalLine(value: unknown): value is JournalLine {
  ...
  if (record.type !== "started" && record.type !== "result") return false
  if (typeof record.key !== "string" || typeof record.agentId !== "string") return false
